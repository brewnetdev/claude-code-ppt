...
| `report` | 비즈니스 리포트, KPI 대시보드, 데이터 슬라이드 | 웜 크림 BG `#FAFAF8`, 티얼 포인트 `#0F766E`, 표 헤더 2px 언더라인 + zebra 행 | `comparison-table`, `callout-summary` (KPI 박스), `references` |

**자동 추론 규칙** (사용자가 템플릿을 명시 안 한 경우):

- MD에 fenced code 블록 ≥ 3 → `presentation`
- MD에 표 ≥ 2 + "Q1/Q2", "KPI", "ROI", "매출", "지표", "성과" 같은 헤딩 → `report`
