...
### 기존 프로젝트 Skills를 GSD executor에 주입
...
이렇게 하면 GSD가 코드를 실행할 때 기존 프로젝트에서 쓰고있는 Spring Boot 패턴, AI 파이프라인 구조, 포인트/등급 규칙을 자동으로 참조한다. (run-ai에 적용한 실제 스킬 executor 예시)

### 기존 .claude/ 파일과의 관계

GSD는 `.planning/` 디렉토리를 사용하고, 프로젝트 파일은 `.claude/`에 있으므로 **충돌 없이 공존**한다.

| | 기존 프로젝트의  `.claude/` 파일 | GSD `.planning/` |
|---|---|---|
