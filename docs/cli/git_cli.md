...
### 원격 브랜치를 현재 브랜치로 병합 (Fetch + Merge)

> 원격 저장소의 **최신 브랜치(주로 `develop` / `main`)를 현재 작업 브랜치로 가져와 병합한다. 기능 개발 중 베이스 브랜치가 앞서 나간 상황에서 PR 머지 전 충돌을 미리 해결하거나, 최신 변경사항을 반영해 계속 작업할 때 사용한다.

**대표 사용 사례**: `feature/ui` 브랜치에서 작업 중인데 `develop`에 다른 팀원이 머지한 변경사항이 생겼을 때, 내 브랜치에 `develop`의 최신 상태를 가져와 반영.

```bash
# 방법 1: fetch + merge (권장 — 로컬 develop 체크아웃 불필요)
git fetch origin develop
git merge origin/develop

# 방법 2: 로컬 develop도 최신화하고 싶다면
git checkout develop
git pull origin develop
git checkout feature/ui
git merge develop

# 방법 3: rebase로 병합 커밋 없이 직선형 히스토리 유지
git fetch origin develop
...
git merge --continue          # merge 사용 시
git rebase --continue         # rebase 사용 시

# 중단하고 원상복구
git merge --abort             # merge 사용 시
git rebase --abort            # rebase 사용 시
```

**머지 후 원격에 반영**
```bash
git push origin feature/ui
# rebase 후 이미 push한 브랜치라면 force push 필요 (공유 브랜치 주의)
git push --force-with-lease origin feature/ui
```

> **주의**:
> - 작업 중 미커밋 변경이 있으면 먼저 `git status`로 확인하고 필요 시 `git stash`로 임시 보관.
> - `rebase` 방식은 이미 push된 브랜치 히스토리를 변경하므로 혼자 사용하는 feature 브랜치에서만 권장. 공유 브랜치는 `merge` 사용.
> - PR을 연 상태라면 develop을 끌어와 머지하면 PR 상에서 "up to date with base" 상태로 갱신되고, 머지 가능 여부도 재평가된다.
