...
## 도구 게이트 (규칙 암기보다 도구 강제 우선)

- **작업 전 스킬 발동**: HTML·SVG·슬라이드·UI 생성/수정 전, 설치된 UI/UX 스킬·플러그인(`anti-ai-slop`, `clean-html`, `svg`, `frontend-design` 등)을 먼저 발동한다. 규칙을 기억에 의존해 적용하지 말고 스킬로 강제한다.
- **디자인 방향 탐색**: 새 시안·룩앤필은 `frontend-design` 계열 플러그인으로 잡되, 그 결과가 위 금지 목록과 충돌하면 **이 문서가 이긴다**.
- **작업 후 자동 검사**: `check-slop` 같은 자동 게이트가 설치돼 있으면 통과할 때까지 재작성한다. 육안 점검은 게이트 통과 후 보조 수단이다.
