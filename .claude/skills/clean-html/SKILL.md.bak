...
description: >
  ...
  규칙은 docs/guide/FRONTEND-GENERATE-HTML-SKILL.md가 단일 출처.
  ...
  that must avoid the generic "AI-generated" look. 생성·수정 후 반드시 check-slop.js로 게이트한다.
...
2. 출력 전 **결정론적 린터**(`scripts/check-slop.js`)로 자가 점검을 통과시킨다. 모델의 눈대중이 아니라 스크립트가 게이트한다.

> 규칙의 단일 출처는 `docs/guide/FRONTEND-GENERATE-HTML-SKILL.md`. 큰 변경 전 반드시 그 파일을 먼저 읽는다. 이 SKILL.md는 운영 체크리스트다.
...
3. **게이트**: `node .claude/skills/clean-html/scripts/check-slop.js <파일>`
...
1. `node .claude/skills/clean-html/scripts/check-slop.js <대상.html>` 로 먼저 스캔.
...
```bash
# 한 파일 또는 여러 파일
node .claude/skills/clean-html/scripts/check-slop.js path/to/page.html
node .claude/skills/clean-html/scripts/check-slop.js a.html b.html c.css
```
...
`check-slop.js`가 자동 점검하지만, 출력 직전 사람이 다시 확인한다. 하나라도 YES면 제거 후 재작성.
