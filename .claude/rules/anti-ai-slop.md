...
- **레이아웃 폭 (문서형 HTML)**: 본문 컨테이너는 뷰포트에 비례해 채운다 — `width: min(96vw, 1680px)` + `margin: 0 auto`, `@media (max-width:1120px)`에서 `width:100%`. **데스크톱 실효 폭 1080px 이상 보장.** 고정 좁은 폭(640~900px) 금지. 표·`pre`·`img`·`svg`는 컨테이너 전폭을 쓰고 개별 `max-width`로 다시 좁히지 않는다. 장문 산문 전용 문서만 `p { max-width: 100ch }` 허용. 슬라이드 덱(1280×720)·모바일 리더는 적용 대상 아님.
